...
Design::Design(i_int_receiver (&chHeader), i_byte_receiver (&chPixel), i_byte_sender (&chData))
    ...
    hw(chHData, chDData),
    sw(chHeader, chPixel, chHData, chDData, chData)
{   /* nothing */
}
...
{   
    { _specc::fork _scc_fork_0(&sw), _scc_fork_1(&hw); _specc::par(
...
#line 661 "tb.cc"
BHDataSend::BHDataSend(int (&HData)[64], i_block_sender (&chHData))
    : HData(HData), chHData(chHData)
{   /* nothing */
}
...
#line 677 "tb.cc"
BDDataRecv::BDDataRecv(i_block_receiver (&chDData), int (&DData)[64])
    : chDData(chDData), DData(DData)
{   /* nothing */
}
...
#line 693 "tb.cc"
JpegEncodeStripe::JpegEncodeStripe(int (&mduHigh), unsigned char *(&stripe), int (&MDUWide), struct huffman_encoder (&DCEhuff), struct huffman_encoder (&ACEhuff), i_block_sender (&chHData), i_block_receiver (&chDData), ISendBits (&chData))
    : mduHigh(mduHigh), stripe(stripe), MDUWide(MDUWide), DCEhuff(DCEhuff), ACEhuff(ACEhuff), chHData(chHData), chDData(chDData), chData(chData),
    handleData(mduWide, mduHigh, stripe, MDUWide, hdata),
    huffmanEncode(qdata, DCEhuff, ACEhuff, chData),
    quantization(ddata, qdata),
    recvDData(chDData, ddata),
    sendHData(hdata, chHData)
{   /* nothing */
}
...
#line 723 "tb.cc"
JpegEncode::JpegEncode(i_byte_receiver (&chPixel), int (&ImageWidth), int (&ImageHeight), struct huffman_encoder (&DCEhuff), struct huffman_encoder (&ACEhuff), i_block_sender (&chHData), i_block_receiver (&chDData), ISendBits (&chData))
    : chPixel(chPixel), ImageWidth(ImageWidth), ImageHeight(ImageHeight), DCEhuff(DCEhuff), ACEhuff(ACEhuff), chHData(chHData), chDData(chDData), chData(chData),
    encodeStripe(mduHigh, stripe, MDUWide, DCEhuff, ACEhuff, chHData, chDData, chData),
...
#line 1123 "tb.cc"
BHDataRecv::BHDataRecv(i_block_receiver (&chHData), int (&HData)[64])
    : chHData(chHData), HData(HData)
{   /* nothing */
}
...
#line 1139 "tb.cc"
BDDataSend::BDDataSend(int (&DData)[64], i_block_sender (&chDData))
    : DData(DData), chDData(chDData)
{   /* nothing */
}
...
#line 1155 "tb.cc"
DctHW::DctHW(i_block_receiver (&chHData), i_block_sender (&chDData))
    : chHData(chHData), chDData(chDData),
    dct(HData, DData),
    recvHData(chHData, HData),
    sendDData(DData, chDData)
{   /* nothing */
}
...
#line 1178 "tb.cc"
HW::HW(i_block_receiver (&chHData), i_block_sender (&chDData))
    : chHData(chHData), chDData(chDData),
    dctHW(chHData, chDData)
{   /* nothing */
}
...
#line 1498 "tb.cc"
JpegSW::JpegSW(i_int_receiver (&chHeader), i_byte_receiver (&chPixel), i_block_sender (&chHData), i_block_receiver (&chDData), i_byte_sender (&chData))
    : chHeader(chHeader), chPixel(chPixel), chHData(chHData), chDData(chDData), chData(chData),
    bitAdapter(chData),
    encode(chPixel, imageWidth, imageHeight, DCEhuff, ACEhuff, chHData, chDData, bitAdapter),
    end(bitAdapter),
...
#line 1549 "tb.cc"
SW::SW(i_int_receiver (&chHeader), i_byte_receiver (&chPixel), i_block_sender (&chHData), i_block_receiver (&chDData), i_byte_sender (&chData))
    : chHeader(chHeader), chPixel(chPixel), chHData(chHData), chDData(chDData), chData(chData),
    jpegSW(chHeader, chPixel, chHData, chDData, chData)
{   /* nothing */
}
