...
#line 553 "tb.h"
    virtual ~IProtocolMaster(void){};

#line 21 "./bus.sc"
    virtual bit<_BITLEN(31,0),false> read(bit<_BITLEN(31,0),false>) = 0;
    virtual void write(bit<_BITLEN(31,0),false>, bit<_BITLEN(31,0),false>) = 0;
private:
};
...
#line 568 "tb.h"
    virtual ~IBusMaster(void){};

#line 108 "./bus.sc"
    virtual void recvBlock(int [64], int) = 0;

#line 107 "./bus.sc"
    virtual void recvInt(int *, int) = 0;

#line 105 "./bus.sc"
    virtual void sendBlock(int [64], int) = 0;

#line 104 "./bus.sc"
    virtual void sendInt(int, int) = 0;
private:
};
...
#line 606 "tb.h"
    virtual ~IBusSlave(void){};

#line 253 "./bus.sc"
    virtual void recvBlock(int *, int) = 0;
...
#line 26 "./bus.sc"
class CSWProtocol : public _specc::channel, public IProtocolMaster
{
private:

#line 26 "./bus.sc"
    _specc::signal<bit<_BITLEN(31,0),false> > (&MWDATA);
    _specc::signal<bit<_BITLEN(31,0),false> > (&MRDATA);
    _specc::signal<bit<_BITLEN(0,0),false> > (&MTSB);
    _specc::signal<bit<_BITLEN(0,0),false> > (&MTAB);
    _specc::signal<bit<_BITLEN(0,0),false> > (&MWDATAOE);
    _specc::signal<bit<_BITLEN(31,0),false> > (&MADDR);
    _specc::signal<bit<_BITLEN(0,0),false> > (&MRWB);
public:

#line 817 "tb.h"
    CSWProtocol(_specc::signal<bit<_BITLEN(31,0),false> > (&MWDATA), _specc::signal<bit<_BITLEN(31,0),false> > (&MRDATA), _specc::signal<bit<_BITLEN(0,0),false> > (&MTSB), _specc::signal<bit<_BITLEN(0,0),false> > (&MTAB), _specc::signal<bit<_BITLEN(0,0),false> > (&MWDATAOE), _specc::signal<bit<_BITLEN(31,0),false> > (&MADDR), _specc::signal<bit<_BITLEN(0,0),false> > (&MRWB));
    virtual ~CSWProtocol(void);
    bit<_BITLEN(31,0),false> read(bit<_BITLEN(31,0),false>);
    void write(bit<_BITLEN(31,0),false>, bit<_BITLEN(31,0),false>);
private:
};

#line 112 "./bus.sc"
class CSWBus : public _specc::channel, public IBusMaster
{
private:
...
#line 112 "./bus.sc"
    ...
    _specc::signal<bit<_BITLEN(31,0),false> > (&MADDR);
    _specc::signal<bit<_BITLEN(0,0),false> > (&MRWB);
    _specc::signal<bit<_BITLEN(0,0),false> > (&INTC);
public:
...
#line 841 "tb.h"
    ...
    virtual ~CSWBus(void);
    void recvBlock(int [64], int);
    void recvInt(int *, int);
    void sendBlock(int [64], int);
...
#line 180 "./bus.sc"
class CHWProtocol : public _specc::channel, public IProtocolSlave
{
private:

#line 180 "./bus.sc"
    _specc::signal<bit<_BITLEN(31,0),false> > (&DB);
    _specc::signal<bit<_BITLEN(0,0),false> > (&DBOE);
    _specc::signal<bit<_BITLEN(0,0),false> > (&TSB);
    _specc::signal<bit<_BITLEN(0,0),false> > (&TAB);
    _specc::signal<bit<_BITLEN(31,0),false> > (&MADDR);
    _specc::signal<bit<_BITLEN(0,0),false> > (&MRWB);
public:

#line 868 "tb.h"
    CHWProtocol(_specc::signal<bit<_BITLEN(31,0),false> > (&DB), _specc::signal<bit<_BITLEN(0,0),false> > (&DBOE), _specc::signal<bit<_BITLEN(0,0),false> > (&TSB), _specc::signal<bit<_BITLEN(0,0),false> > (&TAB), _specc::signal<bit<_BITLEN(31,0),false> > (&MADDR), _specc::signal<bit<_BITLEN(0,0),false> > (&MRWB));
    virtual ~CHWProtocol(void);
    bit<_BITLEN(31,0),false> read(bit<_BITLEN(31,0),false>);
    void write(bit<_BITLEN(31,0),false>, bit<_BITLEN(31,0),false>);
private:
};
...
#line 257 "./bus.sc"
class CHWBus : public _specc::channel, public IBusSlave
{
private:
...
#line 891 "tb.h"
    CHWBus(_specc::signal<bit<_BITLEN(31,0),false> > (&DB), _specc::signal<bit<_BITLEN(0,0),false> > (&DBOE), _specc::signal<bit<_BITLEN(0,0),false> > (&TSB), _specc::signal<bit<_BITLEN(0,0),false> > (&TAB), _specc::signal<bit<_BITLEN(31,0),false> > (&MADDR), _specc::signal<bit<_BITLEN(0,0),false> > (&MRWB), _specc::signal<bit<_BITLEN(0,0),false> > (&INTC));
    virtual ~CHWBus(void);
    void recvBlock(int *, int);
    void recvInt(int *, int);
    void sendBlock(int [64], int);
private:
...
#line 20 "./dct.sc"
    ...
    int r_alu; int r_div; int r_mul;

#line 23 "./dct.sc"
    int r_ram1; int r_ram2;
    int r_sh;
    int rf[32];
};
...
#line 22 "./hw.sc"
    int (&DData)[64]; IBusSlave (&bus);
public:

#line 1113 "tb.h"
    BDDataSend(int (&DData)[64], IBusSlave (&bus));
...
#line 13 "./hw.sc"
    IBusSlave (&bus); int (&HData)[64];
public:

#line 1129 "tb.h"
    BHDataRecv(IBusSlave (&bus), int (&HData)[64]);
...
#line 1182 "tb.h"
    HW(_specc::signal<bit<_BITLEN(31,0),false> > (&DB), _specc::signal<bit<_BITLEN(0,0),false> > (&DBOE), _specc::signal<bit<_BITLEN(0,0),false> > (&TSB), _specc::signal<bit<_BITLEN(0,0),false> > (&TAB), _specc::signal<bit<_BITLEN(31,0),false> > (&MADDR), _specc::signal<bit<_BITLEN(0,0),false> > (&MRWB), _specc::signal<bit<_BITLEN(0,0),false> > (&INTC));
    virtual ~HW(void);
    void main(void);
private:
...
#line 25 "./encode.sc"
    IBusMaster (&bus); int (&DData)[64];
public:

#line 1203 "tb.h"
    BDDataRecv(IBusMaster (&bus), int (&DData)[64]);
...
#line 16 "./encode.sc"
    int (&HData)[64]; IBusMaster (&bus);
public:

#line 1219 "tb.h"
    BHDataSend(int (&HData)[64], IBusMaster (&bus));
...
#line 1360 "tb.h"
    JpegEncodeStripe(int (&mduHigh), unsigned char *(&stripe), int (&MDUWide), struct huffman_encoder (&DCEhuff), struct huffman_encoder (&ACEhuff), IBusMaster (&bus), ISendBits (&chData));
...
#line 1426 "tb.h"
    JpegEncode(i_byte_receiver (&chPixel), int (&ImageWidth), int (&ImageHeight), struct huffman_encoder (&DCEhuff), struct huffman_encoder (&ACEhuff), IBusMaster (&bus), ISendBits (&chData));
...
#line 1547 "tb.h"
    JpegSW(i_int_receiver (&chHeader), i_byte_receiver (&chPixel), IBusMaster (&bus), i_byte_sender (&chData));
    virtual ~JpegSW(void);
    void main(void);
private:
...
#line 11 "./transducer.sc"
class Transducer : public _specc::behavior
{
private:
